...
def _build_system_prompt(self, genome: AgentGenome) -> str:
        ...
        if genome.system_instruction_variant == "expert_coder":
            core = (
                ...
                "Thoroughly address ALL edge cases including empty inputs, negative values, "
                "and cases where no solution exists. {invalid_rule}"
            )
        elif genome.system_instruction_variant == "pedantic_reviewer":
            core = (
                "You are a meticulous code reviewer and software engineer. "
                "Before writing any code, carefully analyze and consider every edge case: "
        ...
        if _AGENT_MEMORY and self.enable_memory:
            prompt += (
                ...
                + _AGENT_MEMORY
                + "\n--- END OF AGENT MEMORY ---\n"
                "Use the insights from the memory to guide your approach and avoid known failure patterns."
            )
